...
from test_AAM import predict_val, predict_test, predict_test_cate
# import test_Rest14
...
class AspectAnything:
    def __init__(
        ...
        **kwargs,
        ...
        if encoder_decoder_type in ["bart", "marian"]:
            config = config_class.from_pretrained(encoder_decoder_name, id2label=id2label, label2id=label2id)
            self.model = model_class.from_pretrained(encoder_decoder_name, config=config)
    ...
    def train(
        ...
        for current_epoch in train_iterator:
            ...
            accuracy, pr, ra, f1 = predict_test(model, self.device, './Laptop-ACOS/laptop_quad_dev_with_none.txt')
            print('Accuracy: '+str(accuracy)+ f'; Precision: {pr}, Recall: {ra}, F1: {f1}')
            if accuracy > best_accuracy:
                self._save_model(self.args.output_dir, model=self.model)
                best_accuracy = accuracy
                # test_acc = predict_test(model, self.device, './Laptop-ACOS/laptop_quad_test_with_none.txt')
                # print(test_acc)
                with open('./acc_record/only_cate_dice.txt', 'a') as f0:
                    f0.writelines('batch: '+str(args.train_batch_size)+' accumulation_steps: '+str(args.gradient_accumulation_steps)+\
                                  ' lr: '+str(scheduler.get_lr()[0])+' epochs: '+str(args.num_train_epochs)+' epoch: '+str(epoch_number)+' val_accuracy: '+str(best_accuracy) + '\n'
                                  )
